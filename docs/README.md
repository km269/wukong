...
> **Go**: 1.26 | **文件**: 241 `.go` (52 `_test.go`) | **包**: 29 内部 + 2 公共 | **许可证**: GNU AGPL-3.0
>
...
Wukong 实现了完整的 ANP 协议栈，使 Agent 之间可以进行标准化的互通：
...
**ANP 能力协商流程**:
1. Agent A 向 Agent B 的 `/anp/capabilities` 发出 GET 请求
2. Agent B 返回 ADP 文档（接口卡列表：A2A/MCP/AG-UI）
3. Agent A 通过 `/anp/meta-protocol` 发送 JSON-RPC 2.0 请求进行能力匹配
4. 双方通过 Meta-Protocol 协商确定通信接口
5. 如启用 E2EE，通过 X25519 密钥交换建立加密会话
