...
7. **评测闭环** — ✅ **已收官（2026-09-04）**：实查发现 `wukong eval` 命令已存在（`internal/cli/eval.go`：evalset JSON 输入 + results 输出，本项的 CLI 部分在早期版本已落地，原计划低估了现状）；`internal/cortex` 测试缺口已清偿——`cortex_test.go` 8 用例（lexical 端到端/向量检索/Reranker 降级/VectorCache）+ `memoryflow_test.go` 3 用例（MemoryFlow 摄取→唤醒→事实晋升全链路、fake planner/extractor 接口注入；GraphFlow 启发式抽取→建图→上下文装配）。**测试驱动出两个真实缺陷修复**：① `Reranker.Rerank` 注释承诺"API 失败回退原始顺序"但实现直接返回错误——已按渐进式降级原则修正；② `GraphFlowService.BuildGraph` 将 entities 请求错发给 `ingest_document` 工具（该工具要求 Content 字段）且未兜底 heuristic 抽取的空 Summary——persist 必然失败，启发式抽取路径（jsonGen=nil）自该代码引入起即不可用——已改为调用 `upsert_entities` + 空 Description 回退 Label。待办仅剩：eval 与 `yao agent test` 的 JSONL 逐条输入格式差异（低优先级格式对齐）。
