...
// NewClient creates a new ARD client with the shared httpclient.
...
func (c *Client) doRequest(ctx context.Context, req *http.Request, cacheKey string, cacheBody []byte) (*http.Response, error) {
	if c.enableCircuit && !c.cb.AllowRequest() {
		return nil, fmt.Errorf("circuit breaker open for registry")
	}

	if c.enableCache {
		if cached, ok := c.cache.Get(cacheKey, cacheBody); ok {
			resp := &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(bytes.NewReader(cached)),
				Header:     make(http.Header),
			}
			resp.Header.Set("X-ARD-Cache", "hit")
			return resp, nil
		}
	}
	...
	if c.enableCircuit && resp.StatusCode >= 500 {
		c.cb.RecordFailure()
	} else {
		c.cb.RecordSuccess()
	}

	if c.enableCache && resp.StatusCode == http.StatusOK {
		bodyBytes, err := io.ReadAll(resp.Body)
		if err == nil {
			c.cache.Set(cacheKey, cacheBody, bodyBytes)
...
func (c *Client) FetchCatalog(ctx context.Context, baseURL string) (*AICatalog, error) {
	...
	resp, err := c.doRequest(ctx, req, url, nil)
	if err != nil {
...
func (c *Client) Search(ctx context.Context, baseURL string, searchReq *SearchRequest) (*SearchResponse, error) {
	...
	resp, err := c.doRequest(ctx, httpReq, url, body)
	if err != nil {
...
func (c *Client) Explore(ctx context.Context, baseURL string, exploreReq *ExploreRequest) (*ExploreResponse, error) {
	url := fmt.Sprintf("%s/api/v1/explore", strings.TrimSuffix(baseURL, "/"))

	body, err := json.Marshal(exploreReq)
	if err != nil {
		return nil, fmt.Errorf("marshal request: %w", err)
	}
